...
(function() {
  ...
  buildArray = function() {
    ...
    if (!(dims != null ? dims.length : void 0)) {
      return null;
    }
  ...
  inferYear = function(termIdP, m, d) {
    var md, th, y;
  ...
  getTOffset = function(t) {
    return t.clone().diff(t.clone().startOf('day'));
  };

  period = ["00:00", "08:00", "09:50", "13:30", "15:20", "17:05", "19:20"].map(function(s) {
    var t;
    t = moment(s, 'HHmm');
    return {
      beginT: getTOffset(t),
  ...
  parseTermId = function(termId) {
    var match;
    if ((match = /(\d{4})-(\d{4})-(\d)/.exec(termId)) == null) {
      return null;
    }
    return {
  ...
  parseWeekStr = function(weekStr) {
    var match, part, ret, s, w, _i, _j, _len, _ref, _ref1, _ref2;
    if (!(part = /(([\d,-]+)|全|前八|后八|单|双)周/.exec(weekStr))) {
      return null;
    }
  ...
  parse_L = function(root, termIdP) {
    var days, i, items, tables, ymd, _i, _len, _results;
    days = root.find('td.doc_title').map(function() {
      var match, matchN, md;
      if ((match = /(\d{4})-(\d{2})-(\d{2})/.exec(this.innerHTML)) == null) {
        throw Error(this.innerHTML);
      }
      matchN = match.slice(1).map(Number);
      --matchN[1];
      return md = moment(matchN);
    });
  ...
  parseLabInfo = function(infoStr) {
    ...
    return {
      labName: infoStr.slice(0, i) || '',
      loc: ((_ref = infoStr.slice(i + 1).match(/([^；]*)；/)) != null ? (_ref1 = _ref[1]) != null ? _ref1.trim() : void 0 : void 0) || ''
    };
  };

  parse_G = function(root) {
  ...
  getOrigin = function(Gr, Gl, L) {
    var it, lastDay, lastItem, lastItems, maxW, p, w, z, _i, _j, _k, _len, _len1, _ref, _ref1;
  ...
  combine = function(G, Lrel, cat, origin) {
    var bin, gi, li, p, rel, w, z, _i, _j, _k, _l, _len, _len1, _ref;
    for (z = _i = 1; _i <= 7; z = _i += 1) {
      for (p = _j = 1; _j <= 6; p = _j += 1) {
        _ref = G[z][p];
        for (_k = 0, _len = _ref.length; _k < _len; _k++) {
          ...
          rel = (w - 1) * 7 + (z - 1);
          if ((bin = Lrel[rel])) {
            for (_l = 0, _len1 = bin.length; _l < _len1; _l++) {
              li = bin[_l];
  ...
  ical = new function() {
    ...
    this.dateStr = function(m) {
      return m.format('YYYYMMDD');
    };
    this.timeStr = function(base, offset) {
    ...
    this.makeRecur = function(oz, gi) {
      var l, list, n, w0, wl, ws;
      ws = gi.week;
      l = ws.length;
      if (l === 1) {
        return '';
      ...
      n = wl - w0 + 1;
      if (l === n) {
        return this.template(ICAL_XRULE, {
          x: 'R',
          n: n
        });
      }
      list = ws.map((function(_this) {
        return function(i) {
          return _this.timeStr(oz.clone().add(i - 1, 'weeks'), gi.beginT);
        };
      })(this));
      return this.template(ICAL_XDATE, {
        x: 'R',
        list: list.join(',')
      });
    };
    this.makeG = function(G, origin) {
      var bin, gi, ow, oz, p, ret, seq, z, _i, _j, _k, _len, _ref;
      ret = [];
      for (z = _i = 1; _i <= 7; z = _i += 1) {
        ...
        for (p = _j = 1; _j <= 6; p = _j += 1) {
          ...
          for (_k = 0, _len = _ref.length; _k < _len; _k++) {
            ...
            bin.push({
              ...
              start: this.timeStr(ow, gi.beginT),
              end: this.timeStr(ow, gi.endT),
              recur: this.makeRecur(oz, gi),
              uid: this.getUid()
            });
          }
        }
        ret = ret.concat(bin.sort(function(a, b) {
          return cmp(a.name, b.name) || cmp(a.start, b.start);
        }).filter(function(x, i, a) {
          var xp;
          xp = a[i - 1];
          return i === 0 || x.name !== xp.name || x.start !== xp.start || x.end !== xp.end;
        }).sort(function(a, b) {
          return a.seq - b.seq;
        }));
      }
      return ret.map((function(_this) {
        return function(x) {
          return _this.template(ICAL_EVENT, x);
        };
      })(this)).join('\n');
    };
    this.makeW = function(origin, nameFactory) {
      var end, start, w;
      return ((function() {
        var _i, _results;
        _results = [];
        for (w = _i = 1; _i <= 16; w = _i += 1) {
          start = origin.clone().add(w - 1, 'weeks');
          end = start.clone().add(1, 'days');
          _results.push(this.template(ICAL_WEEK, {
    ...
    this.make = function(Gr, Gl, origin) {
      ...
      return [
        ICAL_HEADER, this.makeG(Gr, origin), this.makeG(Gl, origin), this.makeW(origin, function(w) {
          return "第" + w + "周";
        }), ICAL_FOOTER
      ].join('\n');
    };
  ...
  get_L = function(autocb) {
    var match, params, resp, ___iced_passed_deferral, __iced_deferrals, __iced_k;
    __iced_k = autocb;
    ...
    (function(_this) {
      ...
      return function() {
        if ((match = /name="token" value="([\da-f]+)"/.exec(resp.responseText)) == null) {
          thucal.ui.log(ERR_MSG_LIST);
        ...
        (function(__iced_k) {
          ...
          autocb($(resp.responseText).filter('a'));
          return;
        });
      };
    })(this));
  ...
  unsafeWindow.thucal = thucal = new function() {
    this.lib = {
      $: $,
      moment: moment,
      saveAs: saveAs
    ...
    this.init = function() {
      ...
      this.ui.button.on('click', (function(_this) {
        return function() {
          return _this.make();
        };
      })(this));
